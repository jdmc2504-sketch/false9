...
import { saveRound } from "@/lib/game-session";
...
const MODE_ENTITY_TYPES: Record<GameMode, EntityCategory[]> = {
  ...
  // Legacy modes kept in type but not shown in UI
...
export default function CreateGamePage() {
  ...
  // When mode changes clear categories — old selections may not apply
  ...
  function handleGenerate() {
    ...
    try {
      const round = generateGameRound({
        mode,
        difficulty,
        filters: { categories, entityTypes: MODE_ENTITY_TYPES[mode] },
        playerNames: playerNames.map((n) => n.trim() || n),
  ...
  // Active filter groups for the current mode
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-32">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {step === "mode" && (
              <section>
                <h1 className="text-3xl font-black uppercase tracking-tight mt-2 mb-1">
                  Select Mode
                </h1>
                <p className="text-xs font-bold uppercase tracking-wider text-muted mb-6">
                  ...
                  {GAME_MODES.map((m) => {
                    ...
                    return (
                      <motion.button
                        ...
                        className={`neon-card p-4 flex flex-col items-start gap-3 text-left transition-all ${
                      ...
                      >
                        ...
                        <div>
                          <p className={`text-sm font-black uppercase tracking-wide leading-tight ${
                            selected ? "text-pitch" : "text-foreground"
                          }`}>
                            {m.label}
                          </p>
                        ...
                        {selected && (
                          <div className="absolute top-3 right-3 w-4 h-4 rounded-full bg-pitch" />
                        )}
                      </motion.button>
                    );
                  })}
              ...
              <section>
                ...
                {filterGroups.length === 0 ? (
                  <div className="neon-card p-6 text-center">
                    <p className="text-muted text-sm">No filters available for this mode.</p>
                  </div>
                ) : (
                  filterGroups.map((group) => (
                    <div key={group.label} className="mb-6">
                      <p className="text-[10px] font-black uppercase tracking-[0.25em] text-muted mb-3">
                        {group.label}
                      </p>
            ...
            {step === "difficulty" && (
              <section>
                ...
                <p className="text-xs font-bold uppercase tracking-wider text-muted mb-6">
                  Controls how clear the False9&apos;s hint is
                </p>
                  ...
                  {DIFFICULTIES.map((d, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {selected && (
                          <div className="w-3 h-3 rounded-full bg-pitch shrink-0" />
                        )}
                      </motion.button>
                    );
                  })}
            ...
            {step === "players" && (
              <section>
                ...
                <p className="text-xs font-bold uppercase tracking-wider text-muted mb-6">
                  3-12 players — pass the phone to each
                </p>
                ...
                <div className="flex items-center justify-center gap-6 mb-6">
                  ...
                  <div className="text-center">
                    <span className="text-5xl font-black text-pitch neon-text">
                      {playerNames.length}
                    </span>
